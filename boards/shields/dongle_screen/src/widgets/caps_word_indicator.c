...
static void caps_word_indicator_set_active(lv_obj_t *label, struct caps_word_indicator_state state) {
    if (state.active) {
        lv_obj_set_style_text_color(label, THEME_COLOR_TEXT_PRIMARY, LV_PART_MAIN);
    } else {
        lv_obj_set_style_text_color(label, THEME_COLOR_TEXT_SECONDARY, LV_PART_MAIN);
    }
}
...
int zmk_widget_caps_word_indicator_init(struct zmk_widget_caps_word_indicator *widget,
    ...
    // Use simple text label for caps word indicator
    lv_label_set_text(widget->obj, "CW");
